...
(function(){
  ...
  /* ONE SWITCH.
     ...
     novax-ai-support and novax-autopilot-brain are written and committed but
     NOT deployed on this project -- the Edge Functions list has only novax-ai
     plus the store-intake functions. While they are missing, pointing at them
     meant every Autopilot message ate a wasted 404 before falling back to
     novax-ai anyway: same answer, same Opus cost, one extra round trip.

     So this is false until they are deployed. Flip it to true after:
         supabase functions deploy novax-ai-support novax-autopilot-brain
     and the free deterministic engine and free Groq/Cerebras/Gemini cascade
     take over, with novax-ai kept as the genuine last resort. Nothing else
     needs to change -- the fallback logic below already handles both cases. */
  var NV_FREE_AI_DEPLOYED = false;
